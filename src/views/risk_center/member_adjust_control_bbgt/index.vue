<template>
    <!-- BBGT会员调控 -->
    <div class="container member-adjust-control-bbgt pr">
...
<script setup name='memberAdjustControlBbgt'>
...
const showData = computed(() => {
    return userData.isHasPermiss('merchant.user-simple-ctrl.ruleList-2') ? tabData : tabData.filter(item => item.value !== 2)
})

onMounted(() => {
    getCommonMeta({ types: 'simpleCtrlType,bgtCtrlPools' }).then(res => {
        metaData = Object.assign(metaData, res.data || {})
        metaData.ctrlPools = metaData.bgtCtrlPools
    })
