<template>
    <!-- BBGT会员调控-数据列表（专业模式）-VIP人数比例配置/自动层级池子配置 -->
        ...
        <div class="profit-pool">
            ...
            <el-form ref="deployFormRef" v-if="state.formData.length" :model="state.formData" :disabled="state.isDisabled">
                <el-table :data="state.formData" style="width: 100%" border>
                    ...
                    <el-table-column v-for="(item,index) in metaData.ctrlPools" :key="item" :label="item.label" min-width="120" align="center">
                        <template #default="{ row, $index }">
                            <span
...
const handleSubmit = (el) => {
    el.validate((valid, fields) => {
        if (valid) {
            ...
            dialog.value.showLoading()
            postControlPoolConfig({ value: JSON.stringify(params), type: props.controlType, gameProvider: 'BGT' }).then(() => {
                getData()
            }).catch(() => {
                dialog.value.closeLoading()
            })
