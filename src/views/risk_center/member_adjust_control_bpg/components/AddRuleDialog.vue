<template>
    ...
    <Dialog ref="dialog" width="550" :title="$t('新增')" top="10vh" buttonCenter @confirm="handleSubmit(deployFormRef)" @cancel="cancel">
        ...
        <div v-if="state.formData.controlType && isShow">
            <div>{{$t('温馨提示：')}}</div>
...
const isShow = computed(() => {
    return !props.metaData.ctrlLoseType.some(item => state.formData.controlType === +item.value)
})
...
const handleSubmit = (el) => {
    el.validate((valid, fields) => {
        if (valid) {
            ...
            if (!isShow) delete params.balanceLimit
