    ...
    <div class="container-main">
        <el-form class="filter-form" inline :model="state.formData">
            ...
            <el-form-item :label="$t('返奖率:')">
                <el-select class="select-box" v-model="state.formData.controlType" clearable :placeholder="$t('请选择返奖率')">
                    <el-option v-for="item in metaData.ctrlLoseType" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
            <el-form-item>
            ...
            <el-form-item v-permiss="'merchant.user-simple-ctrl.addRule-1'">
                <el-button v-throttle type="primary" @click="state.showAddDialog = true">{{$t('新 增')}}</el-button>
            </el-form-item>
            <el-form-item v-permiss="'merchant.user-simple-ctrl.syncRule-1'">
        ...
        <el-table class="table-box" :data="state.tableData" border>
            ...
            <el-table-column :label="$t('返奖率')" width="140" align="center">
                <template #default="{ row }">
                    <el-select :disabled="!userData.isHasPermiss('merchant.user-simple-ctrl.addRule-1')" style="width: 100%" v-model="row.controlType"
                        :placeholder="$t('请选择返奖率')">
                        <el-option v-for="item in metaData.ctrlLoseType" :key="item.value" :label="item.label" :value="+item.value" />
                    </el-select>
                </template>
            </el-table-column>
            <!--  <el-table-column v-if="userData.siteInfo.commissionMode === 2" :label="$t('余额限制')" min-width="140" align="center">
                <template #default="{ row }">
                    <el-input-number v-if="!metaData.ctrlLoseType.some(item => row.controlType === +item.value)" style="width: 100%"
                        v-model="row.balanceLimit" :controls="false" :precision="0" :min="0" :placeholder="$t('限制金额')" />
                </template>
            </el-table-column> -->
            ...
            <el-table-column fixed="right" align="center" :label="$t('操作')" width="200">
                <template #default="{ row }">
                    <el-button v-permiss="'merchant.user-simple-ctrl.syncRule-1'" v-throttle type="primary" text
                        @click="handleSynchronization(row.id)">
                        {{$t('同步返奖率')}}</el-button>
                    <el-button v-permiss="'merchant.user-simple-ctrl.addRule-1'" v-throttle type="primary" text @click="handleEdit(row)">{{$t('修改')}}
                    </el-button>
                    <el-button v-permiss="'merchant.user-simple-ctrl.addRule-1'" v-throttle type="danger" text @click="handleDelete(row.id)">
                        {{$t('删除')}}</el-button>
                </template>
            </el-table-column>
        </el-table>
