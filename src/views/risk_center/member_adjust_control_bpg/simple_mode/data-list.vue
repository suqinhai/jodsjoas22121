<template>
    ...
    <div class="container-main">
        <el-form class="filter-form" inline :model="state.formData">
            ...
            <el-form-item v-permiss="'merchant.user-simple-ctrl.editVipTierCfg-1'">
        ...
        <el-table ref="tableRef" class="table-box" :data="state.tableData" border @selection-change="handleSelectionChange">
            ...
            <el-table-column :label="$t('返奖率')" width="140" align="center">
                <template #default="{ row }">
                    <el-select :disabled="!userData.isHasPermiss('merchant.user-simple-ctrl.editLog-1')" style="width: 100%" v-model="row.controlType"
                        :placeholder="$t('请选择返奖率')">
                        <el-option v-for="item in ( row.asTeam ? metaData.ctrlPools : metaData.ctrlLoseType)" :key="item.value" :label="item.label"
                            :value="+item.value" />
                    </el-select>
                </template>
            </el-table-column>
            ...
            <el-table-column v-if="userData.isHasPermiss('merchant.user-simple-ctrl.editLog-1')" fixed="right" align="center" :label="$t('操作')"
                width="100">
                <template #default="{ row }">
...
const isPermiss = computed(() => {
    return userData.isHasPermiss('admin.user-game-control.quit') || userData.isHasPermiss('admin.user-game-control.over') || userData.isHasPermiss('merchant.user-simple-ctrl.addRule-1')
})
...
// 判断返奖率是否超100
const estimateRtp = (controlType) => {
    return !props.metaData.ctrlLoseType.some(item => controlType === +item.value)
}

// 修改
const handleEdit = ({ userId, username, controlType, balanceLimit }) => {
