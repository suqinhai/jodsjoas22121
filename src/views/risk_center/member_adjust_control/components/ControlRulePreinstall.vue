<template>
    ...
    <div class="container-main">
        <el-form class="filter-form" inline :model="state.formData">
            ...
            <el-form-item v-permiss="'merchant.user-simple-ctrl.syncRule'">
        ...
        <el-table class="table-box" :data="state.tableData" border>
            ...
            <el-table-column :label="$t('返奖率')" width="140" align="center">
                <template #default="{ row }">
                    <el-select :disabled="!userData.isHasPermiss('merchant.user-simple-ctrl.addRule')" style="width: 100%" v-model="row.controlType"
            ...
            <!-- <el-table-column v-if="isShow" :label="$t('余额最小值限制')" width="140" align="center">
                <template #default="{ row }">
                    <el-input-number style="width: 100%" v-model="row.minBalance" :controls="false" :precision="2" :min="0" :placeholder="$t('限制金额')"
                        @blur="inputBlur(row)" />
                </template>
            </el-table-column> -->
            <el-table-column v-if="isShow" :label="$t('余额最大值限制')" width="140" align="center">
                <template #default="{ row }">
                    <el-input-number :disabled="!userData.isHasPermiss('merchant.user-simple-ctrl.addRule')" style="width: 100%"
            ...
            <el-table-column fixed="right" align="center" :label="$t('操作')" width="200">
                <template #default="{ row }">
                    <el-button v-permiss="'merchant.user-simple-ctrl.syncRule'" v-throttle type="primary" text @click="handleSynchronization(row.id)">
                        {{$t('同步返奖率')}}</el-button>
                    <el-button v-permiss="'merchant.user-simple-ctrl.addRule'" v-throttle type="primary" text @click="handleEdit(row)">{{$t('修改')}}
                    </el-button>
                    <el-button v-permiss="'merchant.user-simple-ctrl.addRule'" v-throttle type="danger" text @click="handleDelete(row.id)">
                        {{$t('删除')}}</el-button>
                </template>
            </el-table-column>
        </el-table>
...
const BalanceLimitExplain = defineAsyncComponent(() => import('./BalanceLimitExplain.vue'))
...
// 编辑
const handleEdit = ({ id, controlType, maxBalance, minBalance }) => {
    ...
    // ElMessageBox.confirm(isShow.value ? $t('确定修改当前配置规则的返奖率、余额最小值限制、余额最大值限制吗？') : $t('确定修改当前配置规则的返奖率吗？'), $t('提示'), {
    ElMessageBox.confirm(isShow.value ? $t('确定修改当前配置规则的返奖率、余额最大值限制吗？') : $t('确定修改当前配置规则的返奖率吗？'), $t('提示'), {
        type: 'warning'
    }).then(() => {
        let obj = { id, controlType, gameProvider: 'PGC' }
        ...
        globalVBus.$emit('globalLoading', true)
        editSimpleRuleData(obj).then(res => {
            fetchData()
        }).catch(err => {
            globalVBus.$emit('globalLoading', false)
        })
...
const handleDelete = (id) => {
    ElMessageBox.confirm($t('确定删除当前配置规则吗？'), $t('提示'), {
        ...
        globalVBus.$emit('globalLoading', true)
        deleteSimpleRuleData({ id, gameProvider: 'PGC' }).then(res => {
            fetchData()
        }).catch(err => {
            globalVBus.$emit('globalLoading', false)
        })
...
const handleSynchronization = (id) => {
    ...
    ElMessageBox.confirm(title, $t('提示'), {
        ...
        globalVBus.$emit('globalLoading', true)
        postSyncRuleConfig({ id, gameProvider: 'PGC' }).then(res => {
            fetchData()
        }).catch(err => {
            globalVBus.$emit('globalLoading', false)
        })
