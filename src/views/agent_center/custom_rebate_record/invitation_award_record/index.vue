    ...
    <div class="tabs-main">
        ...
        <el-table :data="state.tableData" class="table-box" border :row-class-name="({row}) => (row.isTotal ? 'total-row': '')">
            ...
            <el-table-column :label="$t('上级代理账号')" min-width="120" align="center">
                <template #default="{ row }">
                    <span class="special pointer" @click="showMemberDetail(row.invite.id)">
                        {{ row.invite?.name || ''  }}
                    </span>
                </template>
            </el-table-column>
            ...
            <el-table-column :label="$t('受邀人')" min-width="120" align="center">
                <template #default="{ row }">
            ...
            <el-table-column prop="vipLevel" :label="$t('VIP等级')" min-width="100" align="center" />
            <el-table-column prop="addTime" :label="$t('领取时间')" min-width="160" align="center" />
            <el-table-column prop="commission" :label="$t('奖励金额')" min-width="100" align="center" />
            <el-table-column prop="ticketType" :label="$t('票券类型')" min-width="100" align="center" />
            <el-table-column prop="ticketName" :label="$t('票券名称')" min-width="120" align="center" />
        </el-table>
...
import { getAgentInviteRewardLog } from '@/api/agent'
...
const fetchData = (val = '') => {
    ...
    getAgentInviteRewardLog(params).then(({ data = {}, ext = {} }) => {
        if (!params.isExport) {
            let { total, list } = data
