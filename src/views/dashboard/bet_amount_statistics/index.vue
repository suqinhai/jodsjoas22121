<template>
    ...
    <div class="dashboard-box" v-loading="state.loading">
        <div class="dashboard-box-title">{{$t('投注金额统计')}}</div>
        <div class="dashboard-box-main">
            ...
            <el-table :data="state.tableData">
                <el-table-column fixed="left" prop="reportTime_" :label="$t('日期')" min-width="100" align="center" />
            ...
            <div class="foot">
                <span>{{$t('最多显示10条')}}</span>
                <span v-if="userData.isHasMenuPermiss('admin.report.gameDailyStat')" class="view-more" @click="handleLookMore">{{$t('查看更多')}}
...
const handleSearch = () => {
    ...
    state.loading = true
    getHomeStatData(params).then(({ data = {}, ext }) => {
...
const handleLookMore = () => {
    ...
    router.push(`/data-report/bet-amount-statistics` + params)
}
</script>
