<template>
    <!-- 运营中心-任务派奖审核 -->
    <div class="container">
        <el-tabs v-model="state.formData.hasReview" @tab-change="fetchData('tab')">
        ...
        <div class="tips-box" v-if="state.formData.hasReview === 1 && !custom">{{$t('注：任务中心注册账号任务开启人工审核后，将在此进行审核派奖。')}}</div>
        <div class="tips-box" v-if="custom">{{$t('注册活动勾选人工审核后，将在此进行审核派奖操作。')}}</div>
...
const custom = JSON.parse(localStorage.getItem('current_merchant_site')).custom
...
onMounted(() => {
    ...
    if (props.merchantId) {
        state.formData.siteId = +props.merchantId
    }
    fetchData()
})
...
const fetchData = (val = '') => {
    ...
    if (state.formData.hasReview == 2) {
        params.status = 3
    }
    params.hasReview = true
