    ...
    <Dialog ref="dialog" width="1200" :title="$t('人工审核')" className="reviewDialog" top="10vh" buttonCenter :isCancel="false" :isShowBottomButton="false" @close="emits('close')">
        <div class="container">
            ...
            <el-table :data="state.tableData" class="table-box" border @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="amount" :label="$t('奖励金额')" min-width="120" align="center" />
                ...
                <el-table-column fixed="right" :label="$t('操作')" width="120" align="center" v-if="state.formData.hasReview === 1">
                    <template #default="{ row }">
                        <el-button v-if="row.status === 2 && userData.isHasPermiss('merchant.promotion-review-log.review')" v-throttle type="primary" text @click="audit(0,row)">{{$t('审核')}}</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
import { postReviewList, postReview, getActiveReviewList, postActiveReview } from '@/api/operation.js'
...
const fetchData = (val = '') => {
    ...
    // 0 新人福利,1 每日任务,2 每周任务,3 活跃度宝箱,4 新拼多多， 12余额救援金
    params.promotionType = props.itemData.type
    params.hasReview = params.hasReview === 1 ? true : false
    if (!params.siteId) delete params.siteId
    delete params.receiveTime
    if (val === 'export') params.isExport = true
    globalVBus.$emit('globalLoading', true)
    getActiveReviewList(params).then(res => {
        if (!params.isExport) {
