<template>
    <!-- 投放管理-广告消耗数据-消耗总金额 -->
	<Dialog ref="dialog" width="900" :title="$t('消耗总金额')" top="10vh" :isShowBottomButton="false" @cancel="emits('close')">
		<div class="container">
			<el-table :data="state.tableData" class="table-box" border>
				<el-table-column prop="reportTime_" :label="$t('日期')" align="center" />
				<el-table-column prop="cost" :label="$t('消耗金额')" align="center" />
				<el-table-column prop="addTime" :label="$t('消耗时间')" align="center" />
...
import { getPurchaseVolumeRrecord } from '@/api/data_center.js'
...
const fetchData = (val = '') => {
    ...
    getPurchaseVolumeRrecord(params).then(res => {
