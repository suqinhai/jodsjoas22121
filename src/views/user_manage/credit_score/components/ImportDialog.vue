<template>
    <!-- 用户管理-信誉管理-导入弹窗 -->
    <Dialog ref="dialog" width="400" :title="$t('导入会员')" top="10vh" :isShowSubmit="false" @cancel="cancel">
...
const hanleBatchAmend = () => {
    ...
    input.onchange = (event) => {
        ...
        reader.onload = async (e) => {
            ...
            list = list.filter(item => item[$t('会员ID')] && +item[$t('信誉分')])
            if (list && list.length) {
                ...
                list.forEach(item => {
                    ...
                    for (let key in keyData) {
                        if (key === 'userId') {
                            obj.userId = +item[$t('会员ID')]
                        } else if (key === 'creditScore') {
                            obj.creditScore = +item[$t('信誉分')]
                        } else {
                ...
                try {
                    ...
                    let res = await postCreditImport({ params: JSON.stringify(params) })
                    ElMessageBox.confirm(res.msg, $t('提示'), {
...
const handleDownload = () => {
    ...
    a.download = $t('批量信誉分模版.xlsx')
    if($locale == 'en'){
        a.href = `/file/member_credit_score_en.xlsx`
    }else if($locale == 'vi'){
        a.href = `/file/member_credit_score_vi.xlsx`
    }else{
        a.href = `/file/member_credit_score.xlsx`
    }
