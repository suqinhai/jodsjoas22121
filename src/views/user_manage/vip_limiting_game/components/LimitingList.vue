...
<template>
    ...
    <div class="container vip-limiting-game">
        ...
        <div class="tips-w">
           <p>{{$t('说明：该列表检测在游戏中(//)的会员余额，达到vip等级上限后进入列表，操作后不可进入游戏(//)。')}}</p> 
           <p>{{$t('系统每隔3分钟检测一次，若高峰人数较多则可能增加检测时间。')}}</p> 
           <p>{{$t('针对vip等级会员在玩控制游戏（(//）时，判断触发当前余额上限时进入列表，触发进入列表的会员不会因游戏输赢低于上限而移除，需要进行人工处理‘踢下线并禁止进入游戏’，操作后会员将被踢出，且不可进入控制游戏；')}}</p> 
