...
import { postTableHeaderConfig } from '@/api/member'
...
onMounted(() => {
    state.dataList = props.tableConfigData.all.map(item => {
        let obj = props.metaData.userTitle.find(its => item === its.value)
        return {
            ...obj,
            disabled: ['merchantId', 'id', 'username', 'activeAmount'].includes(item)
        }
    })
...
const allowDrag = (e) => {
    return !['merchantId', 'id', 'username'].includes(e.data.value)
}

const allowDrop = (e, dropNode, type) => {
    return type !== 'inner' && !['merchantId', 'id', 'username'].includes(dropNode.data.value)
}
...
const handleSubmit = (el) => {
    ...
    postTableHeaderConfig({ titles: JSON.stringify(obj), menuId: props.menuId }).then(res => {
        cancel()
        emits('refreshConfig')
    }).catch(err => {
